...
The placement under `connectomics/decoding/checkpoint/` is broadly sound and need not create a circular dependency. The plan nominally lists the five CLI commands, twelve tests, four ordered mutation/constraint actions, seven invariants, and a design document. It is not yet executable safely, however: the numerical regression oracle is unresolved, several action and verification contracts are underspecified, and the proposed tests could pass without demonstrating a useful repair.
