You are reviewing revision v3 of an implementation plan for a CCC run. You are the CODER: you will
implement this plan, so review it for executability.

You reviewed plan_v0 (11 [major]), plan_v1 (G1-G6), and plan_v2 (H1-H6). Plan_v3 responds to
H1-H6. Your primary job: judge whether H1-H6 are actually fixed, and whether the fixes broke
anything.

Plan_v3's substantive changes: the absolute evidence floor is replaced by a DOMINANCE RATIO
(default 0.6) so no supervoxel with nucleus evidence is ever recorded NONE; three sites now ABORT
on a conflicting proper-id collision instead of resolving to CONFLICT; CONFLICT may now merge with
NONE; the state moved to its own uint8 field so the full uint32 id domain is preserved; and the
hierarchy tests were rewritten with concrete commands.
...
1. Is each of H1-H6 really fixed? Name any that are not.
2. Attack "Bound C" (in a cluster recorded PROPER id i, voxels tagged j != i number at most
   (1 - dominance_ratio) * total). Is the induction actually sound across extraction, load_nuc
   duplicate combination, and merge propagation? Find a counterexample if one exists.
3. Attack Invariant N''. Is there ANY reachable path that joins material from two distinct nuclei
   via an agglomeration merge?
4. Are the three new abort sites correct, or is any of them reachable on legitimate data (which
   would make it a denial-of-service on a long distributed run rather than an invariant check)?
5. Is the plan executable as written - concrete enough to implement without re-deriving design?
   Are V1-V10, especially T1-T4, actually runnable as specified?
6. Anything the revision broke that plan_v2 had right.

Be specific: a finding without a concrete failing case or a concrete missing specification is not
useful at this stage. If the plan is ready, say so plainly rather than manufacturing findings.
...
=========================== BEGIN artifacts/plan_v2.md (superseded, for reference) ===========================
# Plan v2
...
=========================== BEGIN artifacts/plan_v2_review.md (your prior review) ===========================
# Plan v2 Review
...
=========================== END artifacts/plan_v2_review.md ===========================

=========================== BEGIN artifacts/plan_v3.md (UNDER REVIEW) ===========================
# Plan v3
