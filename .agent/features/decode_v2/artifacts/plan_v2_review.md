...
## Questions
- Plan rounds (p2) are exhausted with unresolved [major] findings. These are implementation
  edge-cases (upstream-trace convergence, terminal one-to-one matching, UF-init accumulation,
  area_tol/overlap-threshold binding, --zslice GT-crop + selection, a verification
  inconsistency, and the --mem120G syntax bug) rather than core-design flaws. Normal mode
  blocks for a human decision: (A) proceed to code_v0 folding these findings in as coder
  guidance; (B) switch to auto (APPROVE_AUTO_OVERRIDE) and proceed; (C) extend plan rounds.
