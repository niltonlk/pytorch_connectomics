You are the CCC coder (codex) for stage code_v0 of a CCC run. Implement the approved plan (plan_v4, approved with one minor comment), verify it, and write the code artifact draft. This is PHASE A of code_v0; a PHASE B call will follow after long GPU jobs finish.

## Locations (mandatory)
- Your working root is the dedicated detached git worktree: /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model (HEAD 1546f47ece4777e20bcddd1028fb1bb8908ae02d = run_start_ref). Create all new source files under /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model/dev/ec_model/ and all generated artifacts under /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model/dev/ec_model/runs/.
- CCC run folder (writable): /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model. You may write ONLY these files there: /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/artifacts/code_v0.md.tmp and /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/state/code_v0.patch and /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/state/code_v0.phaseA.notes.md. Do NOT write any .done file, run.md, or review artifact.
- Main checkout /projects/weilab/weidf/lib/pytorch_connectomics is shared with other sessions: read inputs from it by absolute path only; never create or modify files there except inside the run folder above. Another session has uncommitted edits in its connectomics/models/losses/ — that is why imports must be pinned to the worktree (plan_v4 Code-stage isolation).
- Do not create git commits, branches, or stashes. Do not modify tracked files in the worktree. dev/ is gitignored.

## Environment
- Python: /projects/weilab/weidf/lib/miniconda3/envs/pytc/bin/python (torch, h5py, zarr, numba, scipy, cc3d available). Always export OMP_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 MKL_NUM_THREADS=2 NUMBA_NUM_THREADS=2 HDF5_USE_FILE_LOCKING=FALSE on the login node: the user has a hard 400-thread limit and other processes; never start many parallel Python processes on the login node.
- Heavy work (ROI decode of 450M-voxel affinities, mining, any GPU work) must run through Slurm, not the login node. Patterns: CPU: sbatch --account=weilab --partition=short --cpus-per-task=8 --mem=96G --time=03:00:00; GPU: sbatch --account=weilab --partition=short --gres=gpu:l40s:1 --cpus-per-task=8 --mem=64G --time=<cap>. Prefix every sbatch with the env var PREREG=dev/ec_model/PREREG.md (project preregistration gate) and write logs to /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model/dev/ec_model/runs/logs/. Inside job scripts cd to /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model and apply the same thread caps (numba may use the allocated CPUs).
- The cluster queue is busy. Poll squeue/sacct at most every 5 minutes.

## Phase A scope
1. Write PREREG.md first (before any sbatch), then implement every file in plan_v4 "Files and Areas", following plan_v4 exactly, applying the plan_v4_review minor comment: define the protected scope as the main working tree EXCLUDING the designated worktree (/projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model) and the CCC run-artifact directory (/projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model), and audit that scope via the command log instead of checking only <main>/dev/ec_model.
2. Run the unit tests (Verification step 1) on the login node with thread caps; fix until they pass.
3. Real-data curation (Verification steps 2–3) through Slurm CPU jobs on the 3 ROIs; wait for completion (poll), fix failures, rerun. Produce the correctness checks, readiness counts, diagnostics, and montages.
4. If readiness passes, submit the GPU verification chain with afterok dependencies: overfit test (Verification 4–5, cap 1h) -> smoke training (Verification 6, cap 2h) -> smoke evaluation on val A only. Do NOT evaluate split B. Record job IDs. Do not wait for GPU jobs to finish in Phase A beyond confirming they are queued/started without immediate failure.
5. Write /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/state/code_v0.patch per the plan's review delivery contract (git diff --no-index /dev/null <file> for every source/doc file under /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model/dev/ec_model excluding runs/; note that git diff --no-index exits 1 when files differ, which is expected).
6. Write the draft artifact /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/artifacts/code_v0.md.tmp with EXACTLY this structure (headings verbatim):
# Code v0
## Overview
## What Changed
...
## Changes Since Previous Code Version
   - Git Baseline must contain lines: run_start_ref: 1546f47ece4777e20bcddd1028fb1bb8908ae02d and current_head: <output of git -C /projects/weilab/weidf/lib/pytorch_connectomics/.claude/worktrees/ccc-ec_model rev-parse HEAD>, plus the worktree path and worktree git status --short output.
   - Files Changed table lists every file with purpose; also list path, line count, sha256 per file (a second table or list is fine).
   - Verification: every command actually run with exit code and key output; Slurm job IDs, states, runtimes, artifact paths/sizes; unit test counts; correctness/readiness results; diagnostics; montage paths marked "pending human review". For GPU steps not yet finished write "PENDING (Phase B)" with job IDs. Never claim a result you did not observe.
   - Changes Since Previous Code Version: exactly "Initial implementation."
7. Also write /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/ec_model/state/code_v0.phaseA.notes.md: short list of GPU job IDs, expected output paths, and anything Phase B must check or finish.

Keep total source (excluding runs/) at or below ~120 KB. If something in the plan is infeasible or wrong in practice, do not silently deviate: implement the closest correct behavior and record the deviation explicitly in Risks and Unknowns.

When done, reply with a short summary (files, test results, job IDs, blockers).
...
=== artifacts/plan_v4.md (approved plan) ===
# Plan v4
