You are the CCC plan reviewer (coder role) for a single incremental code change in the repository /projects/weilab/weidf/lib/pytorch_connectomics (PyTorch Connectomics). You will later implement this plan, so judge whether it is correct, executable, scoped, and verifiable. This is the FINAL allowed plan version (plan_v2); check that each finding from the plan_v1 review is actually resolved and look for new problems introduced by the revision. Reserve [major] for issues that would make the implementation incorrect, scientifically invalid, unsafe, or unverifiable; implementation-level details that the coder can reasonably decide while coding should be [minor].
...
- <plan_v1 review finding 1..8>: resolved | partially resolved | unresolved — <why>
## Findings
...
=== artifacts/plan_v1_review.md (previous review) ===
# Plan v1 Review
...
=== artifacts/plan_v2.md (version under review) ===
# Plan v2
...
- The first B evaluation atomically creates `runs/val_B_access.json`, keyed by the `eval_sites_B.npz` sha256 and recording the full frozen tuple plus the verdict.
...
Unique-entity counts (unique fragments, unique GT neurons) are reported next to every opportunity count.
Support minima use opportunity counts.

**Aggregation.**
- Primary: pooled (micro) over opportunities. Per-site macro means are secondary.
- 95% intervals: cluster bootstrap over the seeded GT neuron k, 1000 draws, seeded RNG. All opportunities from a site go with its k; a seed-swap pair goes with `min(k, j)`.
...
2. **Label consistency.**
   - For 100% of manifest sites, the first-pass label at the seed voxels equals the recorded owner and GT at the seed voxels equals k.
...
1. **Unit tests** (`pytest dev/ec_model/test_ec_model.py`, CPU, threads ≤2). Synthetic XYZ volumes with known GT, first-pass labels and skeleton graphs.
   - **Mining:**
     ...
     - split edge;
     - contamination with the dominant seed outside the window (`contamination_unseedable`) and inside it;
   ...
   - **Evaluator:**
     ...
     - bootstrap cluster assignment (site → k, pair → min(k, j));
...
4. **Overfit test** (1 L40S, ≤45 min; no augmentation or dropout; 2k steps). The fixed set has 32 examples:
   - **8 designated seed-swap pairs = 16 examples**, both members included. The seed-k member of every pair is a thin (≤45 nm) gap/split site.
   - **16 further examples**, of which ≥8 are thin gap/split sites. Total thin gap/split examples ≥16.
   - The rest cover contamination and controls.
   - Pass: mean Dice ≥ 0.90 over all 32, and ≥ 0.85 on the thin gap/split subset.
5. **Seed-only dependence after overfit**, on the **8 trained pairs**: ≥6 valid pairs (both own-target recalls ≥ 0.5) and median valid-pair IoU ≤ 0.3 → pass. The same metric on 8 **unseen** pairs from other train crops is reported as a diagnostic only.
...
8. **Baseline integrity and delivery:** `git status --short` equals the run-start status (empty). `state/code_v0.patch` covers every source file in the table, with per-file sha256 in code_v0.md.
...
v2 addresses every plan_v1_review finding (the plan_v0_review resolutions from v1 are kept):

1. **[major] Fragment purity not a complete classification.**
   - Replaced by an ordered first-match classification: `nz==0` → unannotated → unique dominant GT (lowest-id tie) → purity test → `pure_d` / `bgdom_pure_d` → everything else `mixed`. The classes are exhaustive and mutually exclusive.
   - Join truth defined for every class.
   - Tests for 60/40, 99/1, 98.9/1.1, all-background, background-heavy (with and without two GT nodes), and ties.
2. **[major] Support depends on model success.**
   - Support minima now count eligible opportunities only: missed nodes, sites, positive and negative candidates, controls, pairs.
   - Joins that never fire give precision `NA`, treated as failing, with recall 0.
   - Explicit verdict precedence: INSUFFICIENT_SUPPORT → KILL → PASS → FAIL.
   - KILL includes "no τ reaches thin recall ≥ 0.10".
   - Zero-output-model test → KILL.
3. **[major] B guard bypassable.**
   - Guard keyed to the `eval_sites_B.npz` sha256 (experiment + manifest).
   - The frozen JSON must carry checkpoint content sha256, τ, config sha256, eval-site sha256 and evaluator sha256, all verified before B data loads.
   - Any second B evaluation is refused regardless of file name, checkpoint or τ. The override rewrites the original verdict to `INVALIDATED`.
   - Tests for a renamed file, a different checkpoint, a hash mismatch, and the override.
4. **[major] Unspecified evaluation choices.**
   - Declared opportunity units: repeats are distinct opportunities, with unique-entity counts reported.
   - Candidate radius = median over in-region aligned nodes, any GT.
   - τ selection objective with tie-breaks and fallback branches, recorded in the frozen JSON.
   - Cluster bootstrap: site → k, pair → min(k, j).
   - Support counted in the same opportunity units.
5. **[major] Alignment mismatches used as truth.**
   - Misaligned nodes (`gt.h5[node] ≠ global_gt_id`) are excluded before purity, mining and metrics, and their edges dropped.
   - Counted by radius bin and split; hard stop if > 2% per ROI.
   - Assertion that no misaligned node reaches manifests or metrics; unit test added.
6. **[major] Mining/sampler edge cases.**
   - Singleton components: anchor = node, length = half boundary edges.
   - Edgeless and boundary-free components → unseedable.
   - Deterministic tie rules for centroid, boundary side and label.
   - Contamination anchor = lowest-index minority node; voxel-only mixed labels are counted without a site.
   - Active weights recomputed after each removal; `SamplerExhausted` gives a readiness failure with non-zero exit.
   - Tests for each.
7. **[major] Mandatory per-type abundance.**
   - Re-derivation and montages use `min(available, requested)`, with absent types reported as `absent`.
   - Readiness minima apply only to thin train sites and thin val-A sites.
   - Montage paths are marked `pending human review` and not claimed as a completed audit.
8. **[major] Overfit pairs not guaranteed trained.**
   - The 32-example overfit set includes both members of the 8 designated pairs (16 examples, seed-k member thin) plus 16 others (≥8 thin), reconciling the ≥16-thin requirement.
   - The seed-dependence pass is measured on the trained pairs; unseen pairs are a diagnostic only.
9. Consistency edits:
   - The contamination target is the voxel-dominant GT (same rule as purity), replacing "most skeleton nodes".
   - The B-access record file is renamed to `runs/val_B_access.json` throughout.
