You are the CCC plan reviewer (coder role) for a single incremental code change in the repository /projects/weilab/weidf/lib/pytorch_connectomics (PyTorch Connectomics). You will later implement this plan, so judge whether it is correct, executable, scoped, and verifiable. This is plan_v3, an extra planning round the user explicitly authorized after the plan_v2 review; it is the final planning round. Check that each finding from the plan_v2 review is actually resolved and look for new problems introduced by the revision. Reserve [major] for issues that would make the implementation incorrect, scientifically invalid, unsafe, or unverifiable; implementation-level details the coder can reasonably decide while coding should be [minor].
...
- <plan_v2 review finding 1..5>: resolved | partially resolved | unresolved — <why>
## Findings
...
=== artifacts/plan_v2_review.md (previous review) ===
# Plan v2 Review
...
=== artifacts/plan_v3.md (version under review) ===
# Plan v3
...
- **Confidence intervals are descriptive only and never enter gate or verdict logic.** All gate thresholds and verdict rules use pooled point estimates.
- **Dependency groups** (frozen per split before any model evaluation, from frozen sites and first-pass labels only):
  - Build a union-find over evaluation units, where a unit is a site, or a seed-swap pair treated as one unit containing both members.
  - Two units are joined if they share any **contributing entity**:
    - the seeded GT neuron k;
    - the swap partner neuron j;
    - any candidate fragment id (first-pass label) evaluated in the unit's join metric;
    - the owner/seed fragment id.
  - Connected components are the dependency groups, stored as `eval_groups_{A,B}.npz` with the group count.
  - Other neurons touched only through the voxel wrong-neuron-inclusion metric are not contributing entities; that residual dependence is documented.
- **Bootstrap:** resample whole dependency groups, 1000 draws, seeded RNG; every opportunity in a unit goes with its group.
- **Group count:** reported with every metric. If fewer than 20 groups exist, the report gives point estimates with `CI: not reported (n_groups < 20)`. In dense tissue, few groups is an expected outcome, not an error.
...
2. **Label consistency.**
   - For 100% of manifest examples, including **each member of every seed-swap pair**: the first-pass label at the seed voxels equals `seed_fragment_label`, that fragment is `pure` for the target neuron (or `mixed` with dominant target for contamination), and GT at the seed voxels equals the target neuron.
...
1. **Unit tests** (`pytest dev/ec_model/test_ec_model.py`, CPU, threads ≤2). Synthetic XYZ volumes with known GT, first-pass labels and skeleton graphs.
   ...
   - **Evaluator:**
     ...
     - dependency groups: overlapping pairs (1,100) and (2,100) and a site seeded by 100 share one group; a candidate fragment repeated across sites seeded by neurons 3 and 4 merges them into one group; bootstrap resamples whole groups; fewer than 20 groups → no CI; CIs are absent from verdict inputs (verdict unchanged when CIs are removed).
...
plan_v3 was authorized by the user ("one more round of planning") after the plan_v2_review block; `plan_rounds` was raised from 2 to 3. It addresses every plan_v2_review finding. The plan_v0 and plan_v1 review resolutions carried in plan_v2 are unchanged.

1. **[major] Bootstrap clusters did not preserve shared-neuron dependence.**
   - Confidence intervals are removed from all gate and verdict logic; decisions use pooled point estimates only.
   - Descriptive CIs now use the reviewer's fix: frozen **dependency groups**. A union-find over evaluation units (sites; seed-swap pairs as single units) links units sharing a seeded neuron, swap partner, candidate fragment or owner/seed fragment.
   - Whole groups are resampled; the group count is reported; no CI below 20 groups.
   - Residual dependence through wrong-neuron voxel inclusion is documented.
   - Tests: overlapping pairs (1,100)/(2,100) plus a site seeded by 100, a candidate repeated across seeded neurons, fewer than 20 groups, and verdict invariance to CI removal.
2. **[minor] Reserve B access before inference.** Exclusive `O_EXCL` creation of `runs/val_B_access.json` with `status: STARTED` after hash checks but before any B data or model evaluation; finalized to `COMPLETED` with the verdict. A crash leaves `STARTED`, which counts as consumed. Tests for concurrent invocations and a failure after reservation.
3. **[minor] Seed-swap owner-label consistency.**
   - Manifests separate `seed_fragment_label` from `conditioning_owner` (0 = owner-free for swap members).
   - Label consistency validates seed fragment, purity and target GT for every example, including each swap member; swap members' M channel is verified all-zero.
4. **[minor] Overfit construction assumed optional populations.**
   - Deterministic construction with a pair-prerequisite check first. Fewer than 8 eligible pairs → `PREREQUISITE_UNAVAILABLE (n_pairs)` for seed dependence, not a learning failure.
   - Contamination and controls are used up to 4 each when available, with thin gap/split replacements otherwise.
   - The composition used is reported.
5. **[minor] Mining statement reconciliation.**
   - Split anchor is node u, the lower-index endpoint (tested with a reversed edge).
   - Contamination test: a dominant part outside the initial window but inside the crop must be seeded via the whole-crop fallback; unseedable only when absent from the whole crop.
