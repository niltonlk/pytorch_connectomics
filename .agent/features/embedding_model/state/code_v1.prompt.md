You are the CCC coder for stage code_v1 in the PyTorch Connectomics (pytc) repository at /projects/weilab/weidf/lib/pytorch_connectomics (your working directory).

code_v1 = triage review_v0 (below), fix every accepted finding, verify, and write the code summary artifact. The approved design is plan_v2 + the binding amendment (already implemented in code_v0); review_v0 is the planner's code review with GPU evidence.
...
- Keep changes scoped to the review_v0 findings. Do not reformat or touch unrelated files.
- Formatting: do NOT run black or isort on multiple files (multi-file black deadlocks in this sandbox). If needed: `black --workers 1 <single file>` on files you changed.
- Do not launch training: no sbatch, srun, or scripts/main.py runs. No GPU on this node: CUDA tests skip; the planner re-measures on GPU in review_v1.
- Python env: `source /projects/weilab/weidf/lib/miniconda3/bin/activate pytc` (conda run is not on PATH). Use OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 for pytest (login-node thread limit).
- Do not claim a test passed unless you ran it; report actual commands and counts.

Required fixes (from review_v0; each must be addressed explicitly in the artifact):
1. Set the EmbeddingMeanLoss term weight to 0.3 in tutorials/neuron_nisb/base_banis+_embed12.yaml (pre-registered rule outcome). Update any test that asserts the configured weight.
2. Speed: rewrite the voxel gathers in connectomics/models/losses/embedding.py to avoid advanced-index autograd backward paths while keeping identical values and gradients: gather valid voxels via flat indices + index_select (e.g. idx = valid.flatten().nonzero().squeeze(1); e = pred[b].reshape(D, -1).index_select(1, idx).T) and use means.index_select(0, inv) instead of means[inv]; keep index_add_ sums. All existing parity/gradient/chunk tests must pass. Add a CPU test with a non-contiguous pred (e.g. permuted view) to guard stride assumptions. Report CPU before/after forward+backward timing on a random 2x12x128^3 input with a realistic label (~40 components per sample); state that GPU timing is unmeasured (planner target: <=0.10 s on L40S).
3. tests/unit/test_embedding_mean_loss.py::test_cuda_mixed_precision: change only the exact-equality assert_close(loss, loss_fn(pred.float(), ...), rtol=0, atol=0) to rtol=1e-5, atol=1e-5.
4. dev/nisb/scripts/train_gate_watch.py: an squeue failure in job_state must not raise out of watch(): treat as unknown state and keep polling; after 3 consecutive failures print "JOB STATE UNAVAILABLE last_step=..." and exit 4 (report-only, no cancel). Add tests (single failure -> continues; 3 consecutive -> exit 4, cancel not called). Keep all existing watchdog behavior and tests.

Verification to run: new/changed unit tests; full `pytest -q tests/unit/test_embedding_mean_loss.py tests/unit/test_embedding_label_pipeline.py dev/nisb/scripts/test_train_gate_watch.py`; regression `pytest -q tests/unit/test_loss_orchestrator.py tests/unit/test_loss_functions.py tests/unit/test_malis_loss.py tests/unit/test_banis_reproduction_transforms.py` (baseline 92 passed, 1 skipped); `git diff --check`.

When done, write the CCC artifact to /projects/weilab/weidf/lib/pytorch_connectomics/.agent/features/embedding_model/artifacts/code_v1.md.tmp with exactly this structure (headings verbatim, each exactly once):
...
In "Changes Since Previous Code Version", map each review_v0 finding (all majors and minors) to what you did (fixed / no action per review / disagreed with reason). Summarize the ACTUAL diff. Paste the FULL current contents of any git-excluded/ignored file you changed (tutorials/neuron_nisb/base_banis+_embed12.yaml, dev/nisb/scripts/train_gate_watch.py, dev/nisb/scripts/test_train_gate_watch.py) plus the full new embedding.py inside ## Implementation Details.
