...
flat within roundoff.** plan_v3 keeps plan_v2's resolved contracts (total
collision-free namespace, exactly-once remap, single marker stage, quarantine +
transitive firewall, `tau_iou_min=0`, single `eps=1e-4` oracle gate) and pins the
last-round scoring spec: a **non-circular raw score**, ambiguity as a separate
**defer** gate, **no caliber floor**, retain-all-candidates for certification, a
**numeric** marker gate, and explicit **structural** namespace + crossing-schema
assertions.
...
and `branch_merge`. **Structural asserts** (hard, independent of GT): (a) each
enumerated node maps to exactly one distinct nonzero `global_id`; (b) background
fixed at 0; (c) remap lookup covers every foreground `(key,label)` the evaluator
...
`comp_a` = the second-highest `raw` among `a`'s other candidate partners on the
face (exclude `b`; 0 if none), `comp_b` similarly; `ambiguity(e) =
max(comp_a, comp_b) / max(raw(e), 1e-6)`, clipped to `[0,1]`. An edge with
`ambiguity(e) > tau_amb = 0.8` (a near-equal competitor exists) is **DEFERRED**
(recorded, not merged) — the design's "defer ambiguous" rule.

**Retain all candidates for certification.** Every generated candidate (both
directions) is written to the certificate table with its cues and decision;
**only mutual-best, non-deferred, floor-passing candidates enter union
processing.**
...
- **Scoring de-circularized (Codex [major]):** split a **raw** score (weighted
  mean of IoU/tangent/caliber/perp, no ambiguity) used for ranking / mutual-best /
  `tau_score`, from **ambiguity** which is now a separate **defer** gate
  (`ambiguity = best-competitor raw / this raw`, clip [0,1], no-competitor→0,
  guard denom 1e-6; defer if `> tau_amb=0.8`). Specified competitor exclusion,
  cross-endpoint aggregation, and that ranks use raw.
- **Caliber floor removed (Codex [major]):** caliber enters only via `raw`; hard
  gates are `tau_tangent`, `tau_score`, mutual-best, ambiguity, firewall (+
  `tau_iou_min=0`).
- **Structural namespace asserts (Codex [minor]):** added one-distinct-id-per-node,
  bg-0-fixed, complete-coverage — beyond the sampled no-edge metric control.
- **Retain all candidates for certification (Codex [minor]):** cert table logs
  every generated candidate with decision; only mutual-best/non-deferred enter
  union processing.
- **Numeric marker gate (Codex [minor]):** `n_onfg/n_inbox ≥ 0.95`; automatic
  fallback source forbidden for this run.
- **Crossing-schema hard assert (Codex [minor]):** all 600 `crossings/*.npz`
  validated (keys + row lengths) before linking.
