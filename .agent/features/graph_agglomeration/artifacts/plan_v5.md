...
flat within roundoff.** plan_v5 keeps all of plan_v4's contracts (total
...
raw score with best-other-partner ambiguity-defer, no caliber floor, numeric
marker gate, crossing-schema assert, one union attempt per undirected pair) and
applies the two internal-consistency fixes from the plan_v4 review: **Verification
3(a) is scoped to the zero-union control** (with root-wise assertions for linked
output), and the certificate decision enum gains **`same_root_noop`**.
...
root sets at decision time)**, decision ∈ {accepted, same_root_noop,
deferred_ambiguous, not_mutual_best, floor_fail, firewall_conflict}, reject_reason.
(`same_root_noop` = a union attempt where `find(u)==find(v)` already; recorded, not
counted as an accepted union or a rejection.)
...
3. **Namespace controls (hard, two-part).** (a) *Structural, under the zero-union
   remap only:* each enumerated node → one distinct nonzero id, bg 0 fixed,
   complete remap coverage. For the *linked* output the assertion is instead
   root-wise: total nonzero mapping, `R(u)==R(v) ⇔ find(u)==find(v)`, and distinct
   ids across distinct roots (accepted unions share an id — node-wise injectivity
   does not hold post-union). (b) *No-edge metric:* build `R` with zero accepted
   unions and run `--remap`; base/oracle/missing/n_skel/gt_len equal the no-remap
   run within `eps`. Either failing ⇒ stop before trusting any link.
...
Two internal-consistency fixes from the plan_v4 review (approach and executability
unchanged and approved; plan rounds p4→p5):

- **Verification 3(a) scoped to the zero-union control (Codex [major]):** the
  node-wise distinct-id assertion now applies only under the zero-union remap; for
  linked output the check is root-wise (`R(u)==R(v) ⇔ find(u)==find(v)`, distinct
  ids across roots), consistent with S1 — it no longer contradicts accepted unions.
- **Certificate enum gains `same_root_noop` (Codex [minor]):** a `find(u)==find(v)`
  union attempt is recorded as `same_root_noop`, not misclassified as accepted or
  rejected.

All plan_v2→v4 contracts are retained (total collision-free namespace, exactly-once
remap, single marker stage + numeric 0.95 gate, quarantine + transitive firewall,
raw score + best-other-partner ambiguity-defer, no caliber floor, crossing-schema
hard check, one union attempt per undirected pair, single `eps=1e-4` oracle gate).
