#!/usr/bin/env python3
"""Extend frozen v5 with scale-stable symmetric long-branch continuations."""
...
V5_ROOT = DEV / "matchguard_error_correction" / "decoder_gtfree_junction_v5_cap12"
ROOT = DEV / "matchguard_error_correction" / "decoder_gtfree_junction_v7"
DEFAULT_BASE_PROPOSALS = V5_ROOT / "frozen_junction_merges.npz"
DEFAULT_BASE_REPORT = V5_ROOT / "frozen_junction_merges.json"
...
def stable_long_tier(
    ...
    hard = (
        np.isfinite(junction["turn_best_min"])
        ...
        & (junction["turn_best_min"] >= args.min_multiscale_turn_deg)
        & (junction["radius_ratio"] >= args.min_radius_ratio)
        & ~junction["spine_veto"].astype(bool)
        & (
            np.minimum(candidate["left_length_nm"][rows], candidate["right_length_nm"][rows])
            >= args.min_segment_length_nm
        )
    ...
    ambiguous = local_ambiguity_mask(
        left,
        right,
    ...
    return hard & ~ambiguous, ambiguous
...
def main() -> int:
    ...
    parser.add_argument("--min-multiscale-turn-deg", type=float, default=110.0)
    parser.add_argument("--min-radius-ratio", type=float, default=0.45)
    parser.add_argument("--min-segment-length-nm", type=float, default=100_000.0)
    parser.add_argument("--min-affinity-mean", type=float, default=0.40)
    parser.add_argument("--min-affinity-ge09-fraction", type=float, default=0.10)
    parser.add_argument("--local-competition-nm", type=float, default=2_000.0)
    parser.add_argument("--max-component-segments", type=int, default=12)
    ...
    long_tier, local_ambiguous = stable_long_tier(candidate, junction, rows, args)
    additional = long_tier & ~base
    groups = [
        (min(int(left[index]), int(right[index])), (index,))
        for index in sorted(np.flatnonzero(additional).tolist())
    ]
    ...
    parameters = {
        "base_v5_cap12": json.loads(str(base_proposal["parameters"].item())),
        "stable_long_v7": tier_parameters,
    }
    ...
    report = {
        "scope": "GT-free v5 plus scale-stable symmetric long-branch continuations",
        "inputs": {
        ...
        "threshold_provenance": (
            "physical 100-um branch scale, 250-nm skeleton sampling, background-derived spine "
            "cut, and unlabeled top-5%-tail interface mean; no evaluation or pseudo-GT"
        ),
        "base_pairs": int(np.count_nonzero(base)),
        "stable_long_edges": int(np.count_nonzero(long_tier)),
        "local_ambiguous": int(np.count_nonzero(local_ambiguous)),
    ...
    if args.freeze:
        ...
        frozen = {
            **report,
            "scope": "GT-free v7 proposal frozen before evaluation",
        ...
        print(f"frozen {int(np.count_nonzero(accepted))} v7 merges sha256={proposal_digest}")
    else:
